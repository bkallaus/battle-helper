...
## 2026-08-01 - Modal/Drawer Focus Management
**Learning:** When implementing temporary custom UI components (modals, drawers, FABs like the Team Drawer) that unmount upon closing, focus natively drops to `document.body`. This breaks keyboard navigation flow for screen readers and power users.
**Action:** To maintain WCAG accessibility, always bind the `Escape` key and inner close buttons to explicitly restore focus to the triggering element using a `useRef` and a small `setTimeout` to ensure the component unmounts before focus is applied.
