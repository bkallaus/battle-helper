...
## 2024-07-11 - Focus Management on Unmounting Conditional UI Elements
**Learning:** In React apps, conditionally rendered action buttons (like a clear-input "✕" button or a modal/drawer "Close" button) naturally unmount upon activation. When the currently focused element unmounts, the DOM standard forces focus back to the `document.body` indiscriminately. This completely disrupts screen reader flow and keyboard navigation continuity, creating an inaccessible trap where the user loses their place in the UI structure.
**Action:** Always maintain a `useRef` directly linked to a resilient, persistent contextual element (such as the adjacent `<input>` being cleared, or the `<button>` that initially opened the drawer). When writing the unmount-triggering `onClick` handler, actively call `.focus()` on the corresponding stable reference, guiding keyboard focus explicitly to the logical next or previous state rather than relying on browser defaults.
