...
**Learning:** Dynamically updated results in TypeChartPanel were not announced by screen readers when users toggled types. Also, clearing the defending species input left keyboard users with focus loss.
**Action:** Applied `role="status"` and `aria-live="polite"` to the result container. Managed focus with a `useRef` to restore focus to the input when the clear button unmounts upon interaction.
## 2023-10-27 - Focus Management for Drawer/Modal Dismissal
**Learning:** When temporary UI components like drawers or modals unmount upon closing, focus natively drops to `document.body`. This creates a jarring experience for keyboard and screen reader users, who must tab back through the entire page to return to their previous position.
**Action:** To maintain WCAG accessibility, always explicitly restore focus to the triggering element (e.g., the FAB that opened the drawer) when the component is explicitly dismissed (via Escape key or internal close button). Use a `useRef` to track the trigger and a small `setTimeout` to apply `.focus()` after unmount. Do not restore focus on implicit dismissals (like clicking outside), as it can steal focus from the user's intended next interaction.
