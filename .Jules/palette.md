...
## 2026-08-03 - Modal/Drawer Focus Restoration\n**Learning:** When implementing temporary custom UI components (modals, drawers, FABs like the Team Drawer) that unmount upon closing, focus natively drops to `document.body`. To maintain WCAG accessibility, always bind the `Escape` key and inner close buttons to explicitly restore focus to the triggering element using a `useRef` and a small `setTimeout` to ensure the component unmounts before focus is applied.\n**Action:** Always implement an `Escape` key listener and restore focus to trigger elements when building custom modals or ephemeral UI components.
