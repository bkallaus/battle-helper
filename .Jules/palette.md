...
## 2024-08-04 - Focus Management on Dynamic Drawers
**Learning:** When managing ephemeral UI components like a sliding side-drawer (e.g. the Team FAB), manually resetting focus is crucial for keyboard users after an interaction completes. However, focus should only be restored to the triggering element (the FAB) when the drawer is *explicitly* closed by user action (like pressing the `Escape` key, or clicking the close/select button). We must avoid forcefully resetting focus upon implicit closing mechanisms, such as "outside clicks", because this creates aggressive focus-stealing bugs that interrupt whatever unrelated UI element the user was trying to interact with.
**Action:** When implementing modaly-like dynamic components, distinguish between explicit dismisses (which get focus-restoration logic via a small unmounting delay `setTimeout(() => trigger.focus(), 0)`) and implicit dismisses (like clicking away) which should not restore focus.
