...
## 2025-07-28 - Focus Management and Keyboard Accessibility for Drawers\n**Learning:** When implementing modals, drawers, or floating action buttons (like the Team Drawer), standard click-outside behaviors often neglect keyboard accessibility. Not returning focus to the triggering element (the FAB) when dismissing the drawer breaks the user's tab order and creates a jarring accessibility experience for keyboard users and screen readers.\n**Action:** Always bind the `Escape` key to close such components. Use a `useRef` hook attached to the triggering element and invoke `.focus()` when the component closes via `Escape` or an explicit close button to ensure WCAG compliant keyboard navigation flow.
