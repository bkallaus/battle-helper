...
## 2026-06-28 - [Focus Management on Conditionally Rendered Inputs]
**Learning:** When using conditionally rendered UI elements (like a 'clear input' button) that unmount upon interaction, explicitly manage focus to prevent it from resetting to the document body. This improves accessibility and keyboard navigation flow.
**Action:** Attach a `useRef` to the associated persistent element (e.g., the text input) and call `.focus()` on it within the unmounting element's event handler.
