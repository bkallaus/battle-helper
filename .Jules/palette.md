...
## 2026-07-16 - Dynamic Calculation Outcomes and Live Regions
**Learning:** When outcomes are dynamically calculated without a page reload or dedicated submit interaction (like Type Chart matchups that update immediately upon input), screen reader users are not informed of the result changing unless focus is manually moved. This hides critical state changes.
**Action:** Apply `role="status"` and `aria-live="polite"` to container elements wrapping dynamically updated calculation results so changes are automatically announced to screen readers without requiring manual focus shifts.
