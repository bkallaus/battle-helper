    ...
    <div className="builder-panel">
      ...
      <div className="form-group">
        ...
        <div className="species-input-container" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
          <div className="input-with-clear" style={{ flex: 1, minWidth: '200px' }}>
            ...
            />
            {config.species && (
              <button type="button" className="clear-input-btn" onClick={() => setConfig({ ...config, species: '', moves: [] })} aria-label="Clear species selection">✕</button>
            )}
          </div>
      ...
      <div className="form-group">
        ...
        <div className="input-with-clear">
          ...
          />
          {config.nature && (
            <button type="button" className="clear-input-btn" onClick={() => setConfig({ ...config, nature: "Serious" })} aria-label="Clear nature selection">✕</button>
          )}
        </div>
          ...
          <div key={stat} className="stat-row">
            ...
            <div className="stat-controls">
              ...
              <button
                ...
                aria-label={config.evs[stat] > 0 ? `Clear ${statNames[stat] || stat} EVs` : `Max ${statNames[stat] || stat} EVs`}
              >
            ...
            {stat !== 'hp' ? (
              <div className="stat-controls">
                ...
                <button
                  ...
                  aria-label={`Increase ${statNames[stat] || stat} boost`}
                >+1</button>
                <button
                  type="button"
                  className="stat-btn"
                  onClick={() => setConfig({ ...config, boosts: { ...config.boosts, [stat]: Math.max(-6, config.boosts[stat] - 1) } })}
                  aria-label={`Decrease ${statNames[stat] || stat} boost`}
                >-1</button>
              </div>
            ) : <div />}
